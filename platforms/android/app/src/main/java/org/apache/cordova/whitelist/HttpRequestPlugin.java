...
import com.run.action.impl.ApplyRecordExecutor;
import com.run.action.impl.CardInfoExecutor;
import com.run.action.impl.ChangePasswordExecutor;
import com.run.action.impl.ConsumtionExecutor;
import com.run.action.impl.DeleteMessageExecutor;
import com.run.action.impl.FreezeCardExecutor;
import com.run.action.impl.GetMessageExecutor;
import com.run.action.impl.GetQuestionExecutor;
import com.run.action.impl.LoginExecutor;
import com.run.action.impl.MessageDealExecutor;
import com.run.action.impl.MessageDetailExecutor;
import com.run.action.impl.RechargePreExecutor;
import com.run.action.impl.SetQuestionExector;
import com.run.action.impl.UnfreezeCardExecutor;
import com.run.action.impl.ValidateQuestionExecutor;
import com.run.action.impl.ZfbRechargeExecutor;
...
//    private static String MESSAGEDEAL = "messagedealaction"; //审核处理接口
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ActionCommand key = ActionCommand.valueOf(action);
        ActionReceiver actionReceiver = new ActionReceiver(action, args, callbackContext);
        ActionInvoker actionInvoker = null;
        switch (key) {
            case LOGIN:
                actionInvoker = new ActionInvoker(new LoginExecutor(actionReceiver));
                break;
            case CARDINFO:
                actionInvoker = new ActionInvoker(new CardInfoExecutor(actionReceiver));
                break;
            case CHANGEPASSWORD:
                actionInvoker = new ActionInvoker(new ChangePasswordExecutor(actionReceiver));
                break;
            case SETQUESTION:
                actionInvoker = new ActionInvoker(new SetQuestionExector(actionReceiver));
                break;
            case VALIDATEQUESTION:
                actionInvoker = new ActionInvoker(new ValidateQuestionExecutor(actionReceiver));
                break;
            case GETQUESTION:
                actionInvoker = new ActionInvoker(new GetQuestionExecutor(actionReceiver));
                break;
            case RECHARGEPRE:
                actionInvoker = new ActionInvoker(new RechargePreExecutor(actionReceiver));
                break;
            case RECHARGE:
                actionInvoker = new ActionInvoker(new ZfbRechargeExecutor(actionReceiver));
                break;
            case GETMESSAGE:
                actionInvoker = new ActionInvoker(new GetMessageExecutor(actionReceiver));
                break;
            case DELETEMESSAGE:
                actionInvoker = new ActionInvoker(new DeleteMessageExecutor(actionReceiver));
                break;
            case MESSAGEDETAIL:
                actionInvoker = new ActionInvoker(new MessageDetailExecutor(actionReceiver));
                break;
            case FREEZECARD:
                actionInvoker = new ActionInvoker(new FreezeCardExecutor(actionReceiver));
                break;
            case UNFREEZEINFO:
                actionInvoker = new ActionInvoker(new UnfreezeCardExecutor(actionReceiver));
                break;
            case COMSUMTIONACTION:
                actionInvoker = new ActionInvoker(new ConsumtionExecutor(actionReceiver));
                break;
            case APPLYRECORD:
                actionInvoker = new ActionInvoker(new ApplyRecordExecutor(actionReceiver));
                break;
            case MESSAGEDEAL:
                actionInvoker = new ActionInvoker(new MessageDealExecutor(actionReceiver));
                break;
        }
